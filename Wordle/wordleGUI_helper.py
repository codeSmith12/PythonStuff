...
correctLetters = []
global lettersInAnswer
lettersInAnswer = []
global curAnswers
curAnswers = answers.copy()
global wrongLetters
wrongLetters= []
...
def removeWrongLetters(curAnswers, wrongLetters):
    print("Wrong letters:",wrongLetters)
    for line in curAnswers:
        for letter in wrongLetters:
            if letter in line and line in curAnswers:

                print("removing",line, "because of", letter)
                curAnswers.remove(line)

    return curAnswers

def removeInvalids(curAnswers, correctLetters):
    answersLeft = []
    print(correctLetters)
    if len(correctLetters) == 1:
        for line in curAnswers:
            if correctLetters[0][1] in line and line.index(correctLetters[0][1]) == correctLetters[0][0]:
                if line in curAnswers:
                    print("adding",line)
                    answersLeft.append(line)
                    # curAnswers.remove(line)
    elif len(correctLetters) == 2:
        for line in curAnswers:
            if correctLetters[0][1] in line and line.index(correctLetters[0][1]) == correctLetters[0][0] and correctLetters[1][1] in line and line.index(correctLetters[1][1]) == correctLetters[1][0]:
                if line in curAnswers:
                    print("adding",line)
                    answersLeft.append(line)
                    # curAnswers.remove(line)
    elif len(correctLetters) == 3:
        for line in curAnswers:
            if correctLetters[0][1] in line and line.index(correctLetters[0][1]) == correctLetters[0][0] and correctLetters[1][1] in line and line.index(correctLetters[1][1]) == correctLetters[1][0] and correctLetters[2][1] in line and line.index(correctLetters[2][1]) == correctLetters[2][0]:
                if line in curAnswers:
                    print("adding",line)
                    answersLeft.append(line)
                    # curAnswers.remove(line)
    elif len(correctLetters) == 4:
        for line in curAnswers:
            if correctLetters[0][1] in line and line.index(correctLetters[0][1]) == correctLetters[0][0] and correctLetters[1][1] in line and line.index(correctLetters[1][1]) == correctLetters[1][0] and correctLetters[2][1] in line and line.index(correctLetters[2][1]) == correctLetters[2][0] and correctLetters[3][1] in line and line.index(correctLetters[3][1]) == correctLetters[3][0]:
                if line in curAnswers:
                    print("adding",line)
                    answersLeft.append(line)
                    # curAnswers.remove(line)
    ...
    # for line in curAnswers:
    #     if line not in answersLeft:
    #         curAnswers.remove(line)
    return answersLeft
def printAnswersLeft():
    global answers, correctLetters, lettersInAnswer, curAnswers, wrongLetters
    # answersLeft = []
    # curAnswers = removeInvalids()
    curAnswers = removeWrongLetters(curAnswers, wrongLetters)

    print(f"\nCurrent Answers left: {curAnswers}\n")

    curAnswers = removeInvalids(curAnswers, correctLetters)
    print(f"\nCurrent Answers left: {curAnswers}\n")

    # curAnswers = answersLeft
    # print("Answers left:", curAnswers)

def removeDuplicates(lst):
    return [t for t in (set(tuple(i) for i in lst))]
def checkAnswer(guess, matrix):
    global turn, answer, kb, correctLetters, wrongLetters
    for i in range(5):
        if guess[i] == answer[i]:
            matrix[turn][i].configure(bg=_from_rgb(green))

            correctLetters.append((i, guess[i]))
            correctLetters=removeDuplicates(correctLetters)
            lettersInAnswer.append(guess[i])
            # if guess[i] in lettersInAnswer:
            #     lettersInAnswer.remove(guess[i])
        elif guess[i] in answer:
            matrix[turn][i].configure(bg=_from_rgb(yellow))
            if guess[i] not in lettersInAnswer:
                lettersInAnswer.append(guess[i])
